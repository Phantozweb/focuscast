...
export default function EpisodeDetailClientPage({ episode, series, relatedEpisodes }: EpisodeDetailClientPageProps) {
  ...
  return (
    <div className="container mx-auto py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 lg:gap-12 px-0 sm:px-4 md:px-0">
        {/* Main Content Column */}
          ...
          <div className="bg-card p-4 sm:p-6 rounded-lg shadow-lg border">
              ...
              <div className="flex-grow">
                ...
                {series && (
                  <Link href={`/series/${series.id}`} passHref legacyBehavior>
                    <Badge variant="secondary" className="hover:bg-accent cursor-pointer mb-3 text-sm sm:text-sm">{series.title}</Badge>
                  </Link>
                )}
                {episode.episodeNumber && <Badge variant="outline" className="mb-3 ml-1 text-sm sm:text-sm">Episode {episode.episodeNumber}</Badge>}
            ...
            <div className="flex flex-col sm:flex-row gap-3 mb-4">
              <Button
                ...
                className="flex-1 text-base py-6 sm:py-2 sm:text-sm"
              >
